...
export function PreviewPanel({ projectId, runtimeError, onDismiss, onFix }: PreviewPanelProps) {
  const [previewUrl, setPreviewUrl] = useState<string | null>(() => {
    ...
    const savedUrl = localStorage.getItem(PREVIEW_URL_KEY);
    if (savedUrl && savedUrl.startsWith("http://") && window.location.protocol === "https:") {
      return savedUrl.replace("http://", "https://");
    }
    return savedUrl;
  });
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      let newPreviewUrl = response.previewUrl;
      if (newPreviewUrl && newPreviewUrl.startsWith("http://") && window.location.protocol === "https:") {
        newPreviewUrl = newPreviewUrl.replace("http://", "https://");
      }
