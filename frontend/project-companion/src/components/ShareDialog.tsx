                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                            ...
                            {members.map(member => {
                                ...
                                const displayName = isCurrentUser ? "You" : (member.name || member.username || (member.role === 'OWNER' ? 'Project Owner' : 'Unknown User'));
                                const displayUsername = isCurrentUser ? (member.username || currentUser?.username || "") : (member.username || (member.role === 'OWNER' ? 'Owner' : ''));
                                const displayInitial = isCurrentUser ? "YOU" : (member.name ? member.name.charAt(0).toUpperCase() : (member.username ? member.username.slice(0, 2).toUpperCase() : (member.role === 'OWNER' ? 'OW' : '?')));
